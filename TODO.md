...
- [ ] **LLMへの価格推移インプット (日足スナップショット)**
  - 現状: question・現在のYES価格・BTC/ETH価格・ニュースのみ
  - 追加: 過去7〜14日の日足終値スナップショットを文字列でcontextに追加
    例: `"価格推移(日足): 3/12=0.41, 3/13=0.43, 3/15=0.62, 3/18=0.62"`
  - サマリー(最高/最低/変化率)だけでは急騰タイミング等の形が伝わらないため日足を使う
  - 分足・時足の羅列は不要。日足7〜14本で形が読める
  - **LLMが読み取れること:**
    - 急騰後に安定 → ニュース織り込み済み
    - じわじわ上昇中 → まだ情報が浸透していない
    - 急落後に反発 → 過剰反応からの回復
  - **実装場所:** `analyst/ensemble.py` の `analyze()` 内
    - `prices` (1分足) はすでに渡されているので、60本ごとに間引いて日足相当を作る
    - `llm_analyst.analyze_market()` の `context["price_history"]` として渡す
  - **`analyst/llm_analyst.py`:** `context` に `price_history` キーを追加してプロンプトに埋め込む
