...
| コミット | 内容 |
|---|---|
| `aaf2fff` | debug: Gamma API conditionId/outcomes 値をデバッグ出力 (調査中) |
| `c9b81e5` | fix: get_market を conditionId 手動突合方式に変更 (Gamma API クエリフィルタ無効のため) |
...
## 🔍 調査中: 自動解決 (market resolution) が機能しない

### 現状

Polymarket の Gamma API を使った自動ポジションクローズが正しく動作しない。

**これまでに判明したこと:**

1. **conditionId フィルタが効かない**
   - `GET /markets?conditionId=0x...` → フィルタ無視、別マーケット (Biden/Coronavirus) を返す
   - 手動突合 (`closed=true` で100件取得してフィールドで比較) を試みたが…

2. **Gamma API のレスポンススキーマが想定と異なる**
   - 返ってくるキー: `['id', 'question', 'conditionId', 'slug', 'endDate', 'category', 'liquidity', 'outcomes', ...]`
   - `closed` / `active` / `outcomePrices` / `resolutionResult` フィールドが **存在しない**
   - train_ml.py で使っているフィールドと異なる → API エンドポイントかパラメータが違う可能性

3. **conditionId の突合も失敗**
   - 返ってきたマーケット群の conditionId が position の market_id と一致しない
   - → positions の `market_id` と Gamma API の `conditionId` が別フォーマットの可能性

4. **VOID マーケット (outcomePrices=["0","0"]) の扱い**
   - Miami Open は Polymarket UI 上「紛失」だが Gamma API は [0,0] を返していた
   - [0,0] は「未確定」と「VOID」の両方で使われ区別不可 → 現在は無視

5. **誤クローズの経緯**
   - `active=True` + `closed=True` + `outcomePrices=["0","0"]` → VOID 判定で全ポジション誤クローズ (×2回)
   - positions.json を手動で open に復元済み

### 次にやること

- `[market debug]` のログで `conditionId` の値と `outcomes` の構造を確認
- Gamma API の正しいエンドポイント・パラメータを特定
  - `?order=closedTime` を付けると outcomePrices が返るか試す
  - `/events` エンドポイントで market_id 突合を試す
- CLOB API (`/last-trade-price` など) でトークン価格を取得して解決判定する代替案も検討

### 暫定対応

現状は自動解決が無効状態に近い (outcomePrices が明確な [1,0] / [0,1] のみ対応)。
Miami Open など「紛失」状態のポジションは **手動で positions.json を編集** してクローズ。
