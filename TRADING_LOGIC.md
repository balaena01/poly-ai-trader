...
### 4. ポジションサイジング (Half Kelly)

| 状態 | Kelly 分率 |
|---|---|
| LLM skill 未計測 (解決済み < 20件) | base × **1.0** (キャリブレーション中もフルKelly) |
| skill > 0.10 (正のスキル) | base × **1.0** (通常) |
| skill 0.0 ～ 0.10 | base × **0.75** |
| skill -0.05 ～ 0.0 | base × **0.50** |
...
max_position_pct: float = 0.20   # 1ポジション最大 20%
